---
name: asyntai
description: Use when the user wants to manage their Asyntai AI support agent - its knowledge base, instructions, conversations, leads, live chats or plan - or when the asyntai MCP server needs to be connected.
---

# Asyntai

Asyntai puts an AI support agent on the user's website. The `asyntai` MCP
server in this plugin gives you 54 tools for that agent.

## First use: sign in

The server uses OAuth. If a tool call returns an authentication error, tell
the user to run `/mcp`, choose `asyntai`, and pick **Authenticate**. A browser
